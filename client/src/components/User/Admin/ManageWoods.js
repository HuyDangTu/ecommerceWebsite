...
import { getTypes, addType } from '../../../actions/product_actions'
import "./manageWood.scss";
class ManageWoods extends Component {
    ...
    submitForm = (event) => {
        ...
        if (formIsValid) {
            console.log(dataToSubmit);
            this.props.dispatch(addType(dataToSubmit, existingTypes))
                .then(response => {
                    if (this.props.products.addType) {
